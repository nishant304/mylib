...
import com.wecamchat.aviddev.subfragment.BrowserFriendFragment;
...
public class CircularImageView extends ImageView {
    ...
    public CircularImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attributes.getBoolean(R.styleable.CircularImageView_border, true)) {
            if (ProfileFragment.setProfileFlag
                    || BrowserFriendFragment.setFriendFlag) {

                defaultBorderSize = (int) (4 * getContext().getResources()
                        .getDisplayMetrics().density + 0.5f);
                ProfileFragment.setProfileFlag = false;
                BrowserFriendFragment.setFriendFlag = false;
            } else {
    ...
    public void setBorderWidth(int borderWidth) {
        Log.i("Profile", "My Profile first" + borderWidth);
        this.borderWidth = borderWidth;
        Log.i("Profile", "My Profile " + borderWidth);
        this.requestLayout();
        this.invalidate();
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (image != null) {
            ...
            canvas.drawCircle(circleCenter, circleCenter,
                    ((canvasSize - (borderWidth * 2)) / 2) - 3.0f, paintBorder);
            canvas.drawCircle(circleCenter, circleCenter,
                    ((canvasSize - (borderWidth * 3)) / 2), paint);
        } else {
            canvasSize = canvas.getWidth();
            if (canvas.getHeight() < canvasSize)
                canvasSize = canvas.getHeight();
            int circleCenter = (canvasSize) / 2;
            canvas.drawCircle(circleCenter, circleCenter,
                    ((canvasSize - (borderWidth * 2)) / 2), paintBorder);
        }
    }
